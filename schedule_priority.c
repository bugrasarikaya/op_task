...
struct node *pickNextTask(struct node *head){ //Returns the node which has maximum priority from task list.
    struct node *ptr=head;
    int max_priority=ptr->task->priority;
    int max_id=ptr->task->tid;
    while(ptr!=NULL){
        if(ptr!=head){
            if(max_priority<ptr->task->priority){ //Finds maximum priority and task id of the task which has this maximum priority.
                max_priority=ptr->task->priority;
                max_id=ptr->task->tid;
            }
            else if(max_priority==ptr->task->priority)if(max_id>ptr->task->tid)max_id=ptr->task->tid; //If priorities are same, it implements the FCFS.
        }
        ptr=ptr->next;
    }
    if(ptr!=head)ptr=head;
    while(ptr->task->tid!=max_id)ptr=ptr->next; //Finds the node which has maximum priority.
    return ptr;
}
void schedule(){ //Runs tasks from the list according to their priority by using priority scheduling algorithm.
    int time=0;
    int task_count=0;
